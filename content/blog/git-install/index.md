...
| resetのモード | HEADの位置 | インデックス<br />（git addされたファイルの状態） | ワークツリー<br />（ローカルリポジトリの状態） |
| ---- | ---- | ---- | ---- |
...
* Soft：コミットだけなかったことにする（ローカルリポジトリ上に変更はそのまま残る）
* Mix：ローカルリポジトリの変更はそのまま残る
* Hard：指定したコミットを完全になかったことにする
